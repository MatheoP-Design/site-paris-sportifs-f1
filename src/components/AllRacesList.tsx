...
interface AllRacesListProps {
  onSelectRace: (race: any) => void;
}

export function AllRacesList({ onSelectRace }: AllRacesListProps) {
  const [races, setRaces] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [timeRemainingMap, setTimeRemainingMap] = useState<Record<string, any>>({});
  ...
  useEffect(() => {
    ...
    const updateCountdowns = () => {
      ...
      const newTimeMap: Record<string, any> = {};
      ...
      races.forEach((race) => {
        ...
        if (distance < 0) {
          newTimeMap[race.id] = { isPast: true };
        } else {
          newTimeMap[race.id] = {
  ...
  const formatCountdown = (time: any) => {
    if (!time) return '';
    if (time.isPast) return 'Terminé';

    if (time.days > 0) {
      return `Dans ${time.days}j ${time.hours}h`;
    } else if (time.hours > 0) {
      return `Dans ${time.hours}h`;
    } else {
      return 'Aujourd\'hui';
    }
  };
      ...
      {upcomingRaces.length > 0 && (
        <div className="mb-16">
          <div className="mb-8">
            ...
            <h2>Prochains Grands Prix 2026</h2>
            <p className="text-muted-foreground">
              Cliquez sur un GP pour voir les pilotes et placer vos paris
            </p>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {upcomingRaces.map((race, index) => {
              ...
              return (
                ...
                >
                  <Card
                    className="group overflow-hidden border-border hover:border-primary/50 transition-all cursor-pointer h-full flex flex-col"
                  ...
                  >
                    ...
                    <div className="relative h-48 overflow-hidden">
                      ...
                      {time && (
                        <Badge
                          variant={time.isPast ? 'destructive' : 'default'}
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {pastRaces.reverse().map((race, index) => {
              return (
                ...
                >
                  <Card
                    className="group overflow-hidden border-muted bg-muted/5 hover:border-muted-foreground/30 transition-all cursor-pointer opacity-60 hover:opacity-100"
                    onClick={() => onSelectRace(race)}
                  >
